...
action = function( host, port )
  ...
  for username, password in pairs(users) do
    ...
    --db=nmap_args.db
	  sqly=string.format("use %s;DESC %s;", nmap_args.db,nmap_args.tab)
      local status, rs = mysql.sqlQuery( socket, sqly )--use mysql;show tables;use mysql;show create table servers;use mysql;DESC servers;
	  --print(status)
	  --print(rs,status)
	  result=rs
      --if status then
